  ...
  virtual task run_phase(uvm_phase phase);
    ...
    forever begin
      ...
      if(expdata.pwrite == apb_transaction::WRITE) begin
        ...
        if(mem[expdata.addr] == expdata.data) begin
          `uvm_info("APB_SCOREBOARD",$sformatf("------------ READ DATA Match ------------"),UVM_LOW)
          `uvm_info("",$sformatf("Addr: %0h",expdata.addr),UVM_LOW)
          `uvm_info("",$sformatf("Expected Data: %0h Actual Data: %0h",mem[expdata.addr],expdata.data),UVM_LOW)
        end
        else begin
